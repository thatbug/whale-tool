package org.thatbug.whale.core.secure.utils;
...
import org.thatbug.whale.core.launch.constant.TokenConstant;
import org.thatbug.whale.core.secure.TokenInfo;
import org.thatbug.whale.core.secure.WhaleUser;
import org.thatbug.whale.core.secure.constant.SecureConstant;
import org.thatbug.whale.core.secure.exception.SecureException;
import org.thatbug.whale.core.secure.provider.IClientDetails;
import org.thatbug.whale.core.secure.provider.IClientDetailsService;
import org.thatbug.whale.core.tool.utils.*;
 ...
 *
 * @author qzl
 * @date 15:24 2019/9/17
 */
public class SecureUtil {
    private static final String BLADE_USER_REQUEST_ATTR = "_WHALE_USER_REQUEST_ATTR_";
     ...
     * @return WhaleUser
     */
    public static WhaleUser getUser() {
        HttpServletRequest request = WebUtil.getRequest();
        if (request == null) {
            return null;
        }
        // 优先从 request 中获取
        Object whaleUser = request.getAttribute(BLADE_USER_REQUEST_ATTR);
        if (whaleUser == null) {
            whaleUser = getUser(request);
            if (whaleUser != null) {
                // 设置到 request 中
                request.setAttribute(BLADE_USER_REQUEST_ATTR, whaleUser);
            }
        }
        return (WhaleUser) whaleUser;
    }
     ...
     * @return WhaleUser
     */
    public static WhaleUser getUser(HttpServletRequest request) {
        ...
        WhaleUser whaleUser = new WhaleUser();
        whaleUser.setClientId(clientId);
        whaleUser.setUserId(userId);
        whaleUser.setTenantId(tenantId);
        whaleUser.setAccount(account);
        whaleUser.setRoleId(roleId);
        whaleUser.setRoleName(roleName);
        whaleUser.setUserName(userName);
        return whaleUser;
    }
    ...
    public static Integer getUserId(HttpServletRequest request) {
        WhaleUser user = getUser(request);
        return (null == user) ? -1 : user.getUserId();
    }
    ...
    public static String[] extractAndDecodeHeader() {
        ...
        int index = token.indexOf(StringPool.COLON_ESCAPE);
        if (index == -1) {
            throw new RuntimeException("Invalid basic authentication token");
        } else {
            return new String[]{token.substring(0, index), token.substring(index + 3)};
        }
    }
